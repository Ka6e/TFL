...
Если файл drawio/ps_vm.png отсутствует, ниже приведена эквивалентная диаграмма в формате mermaid:

```mermaid
flowchart LR
    IP[InstructionPointer\nint _instructionPointer]
    Prog[Program\nIReadOnlyList<Instruction>]
    Eval[EvaluationStack\nStack<Value>]
    Locals[locals\nDictionary<string, Value>]
    Vm[PsVm\nfetch/decode/execute\nRunProgram()]
    Exit[ExitCode\nint _exitCode]
    Builtin[BuiltinFunctions\nPrint/Input/Convert/Strings]
    Env[Environment\nstdin / stdout]
...
13. `Halt`  
    Останавливает VM.  
    Снимает `EVAL[^1]` и использует его как `ExitCode` (целое число), которое возвращает `RunProgram`.[📷 ps_vm.png]
