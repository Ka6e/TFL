# Виртуальная машина Swiston

Виртуальная машина Swiston выполняет инструкции по определённым правилам, тем самым обеспечивая возможность вычисления любой программы на Swiston.

## Иллюстрация

Иллюстрация состояния виртуальной машины языка Swiston в момент выполнения программы:

```mermaid
graph LR
    PROG["Список инструкций<br/>_instructions"]
    IP["Указатель инструкций<br/>_instructionPointer"]
    EVAL["Стек вычислений<br/>_evaluationStack"]
    VARS["Таблица переменных<br/>_variables"]
    VM["VirtualMachine<br/>RunProgram()"]
    BF["BuiltinFunctions"]
    ENV["stdin / stdout<br/>IEnvironment"]
    RET["Стек возвратов<br/>_returnStack"]

    PROG --> VM
    IP --> VM
    EVAL --> VM
    VARS --> VM
    RET --> VM
    VM --> BF --> ENV

## Модель виртуальной машины

Структура виртуальной машины Swiston состоит из следующих элементов:

1. Список инструкций, составляющих программу;
2. Указатель на следующую инструкцию;
3. Стек значений, используемый для вычисления выражений;
4. Таблица переменных, хранящая все переменные программы;
5. Стек возвратов, хранящий контексты вызовов функций.

## Инструкции

Каждая инструкция программы содержит:

1. Код инструкции из набора инструкций;
2. Значение операнда (необязательное).

## Набор инструкций

Каждая инструкция может иметь один операнд, который хранится вместе с инструкцией.

Условные обозначения в списке инструкций:

- Операнд указывается в квадратных скобках, например: `[Value]`;
- Вершина стека вычислений обозначается `EVAL[^1]`, следующее за ней значение — `EVAL[^2]` и так далее;
- Каждая инструкция имеет свой номер (индекс), нумерация инструкций начинается с 0.

Список инструкций:

1. `Push [Value]` — добавляет значение на стек вычислений
    - Операнд хранит добавляемое значение

2. `Pop` — удаляет значение со стека вычислений

3. `DefineVar [Name]` — записывает в новую переменную значение `EVAL[^1]`
    - Операнд хранит имя переменной
    - Убирает значение со стека вычислений
    - Бросает исключение, если переменная с таким именем уже объявлена

4. `StoreVar [Name]` — записывает в ранее созданную переменную значение `EVAL[^1]`
    - Операнд хранит имя переменной
    - Убирает значение со стека вычислений
    - Бросает исключение, если переменная не была объявлена

5. `LoadVar [Name]` — читает значение переменной
    - Операнд хранит имя переменной
    - Помещает результат в стек вычислений
    - Бросает исключение, если переменная не была объявлена

6. `Add` — выполняет бинарную операцию сложения
    - Вычисляет `EVAL[^2] + EVAL[^1]`
    - Для типа `string` выполняет конкатенацию строк
    - Для типов `int` и `float` выполняет арифметическое сложение
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

7. `Subtract` — выполняет бинарную операцию вычитания
    - Вычисляет `EVAL[^2] - EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

8. `Multiply` — выполняет бинарную операцию умножения
    - Вычисляет `EVAL[^2] * EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

9. `Divide` — выполняет бинарную операцию деления
    - Вычисляет `EVAL[^2] / EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

10. `Modulo` — выполняет бинарную операцию остатка от деления
    - Вычисляет `EVAL[^2] % EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

11. `And` — выполняет бинарную операцию логического «И»
    - Вычисляет `EVAL[^2] && EVAL[^1]`
    - Поддерживаемый тип: `bool` (результат — `bool`)
    - Для типа `int` результат — `1` если оба не ноль, иначе `0`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

12. `Or` — выполняет бинарную операцию логического «ИЛИ»
    - Вычисляет `EVAL[^2] || EVAL[^1]`
    - Поддерживаемый тип: `bool` (результат — `bool`)
    - Для типа `int` результат — `1` если хотя бы одно не ноль, иначе `0`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

13. `Not` — выполняет унарную операцию логического отрицания
    - Вычисляет `!EVAL[^1]`
    - Поддерживаемый тип: `bool` (результат — `bool`)
    - Для типа `int` результат — `1` если ноль, иначе `0`
    - Снимает значение со стека вычислений, результат помещает в стек вычислений

14. `Negate` — выполняет унарную операцию смены знака
    - Вычисляет `-EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`
    - Снимает значение со стека вычислений, результат помещает в стек вычислений

15. `Equal` — выполняет бинарную операцию «равно»
    - Вычисляет `EVAL[^2] == EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`, `string`, `bool`
    - Результат — `bool` (`true` если равны, `false` если нет)
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

16. `NotEqual` — выполняет бинарную операцию «не равно»
    - Вычисляет `EVAL[^2] != EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`, `string`, `bool`
    - Результат — `bool` (`true` если не равны, `false` если равны)
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

17. `Less` — выполняет бинарную операцию «меньше»
    - Вычисляет `EVAL[^2] < EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`, `string`
    - Результат — `bool`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

18. `LessOrEqual` — выполняет бинарную операцию «меньше или равно»
    - Вычисляет `EVAL[^2] <= EVAL[^1]`
    - Поддерживаемые типы: `int`, `float`, `string`
    - Результат — `bool`
    - Снимает два значения со стека вычислений, результат помещает в стек вычислений

19. `Jump [Address]` — выполняет безусловный переход
    - Операнд хранит адрес (индекс) следующей инструкции
    - Устанавливает указатель инструкций равным операнду

20. `JumpIfTrue [Address]` — выполняет условный переход
    - Операнд хранит адрес (индекс) следующей инструкции
    - Снимает `EVAL[^1]` со стека вычислений
    - Выполняет переход, если `EVAL[^1]` — `true` (или не ноль для `int`)

21. `JumpIfFalse [Address]` — выполняет условный переход
    - Операнд хранит адрес (индекс) следующей инструкции
    - Снимает `EVAL[^1]` со стека вычислений
    - Выполняет переход, если `EVAL[^1]` — `false` (или ноль для `int`)

22. `Call [Address]` — выполняет вызов функции
    - Операнд хранит адрес (индекс) первой инструкции функции
    - Сохраняет текущий указатель инструкций и таблицу переменных в стек возвратов
    - Переходит по адресу операнда

23. `Return` — выполняет возврат из функции
    - Восстанавливает указатель инструкций и таблицу переменных из стека возвратов

24. `StoreResult` — сохраняет результат выполнения программы
    - Снимает `EVAL[^1]` со стека вычислений
    - Сохраняет его как результат работы программы

25. `CallBuiltin [Code]` — вызывает встроенную функцию
    - Операнд хранит целочисленный номер встроенной функции (`BuiltinFunctionCode`)
    - Аргументы передаются через стек, возвращаемое значение (если есть) помещается в стек вычислений

26. `PushVars [Depth]` — создаёт новую область видимости
    - Операнд хранит глубину родительской таблицы переменных (0 — глобальная область)
    - Создаёт новую таблицу переменных, связывая её с родительской на указанной глубине

27. `PopVars` — удаляет текущую область видимости
    - Восстанавливает родительскую таблицу переменных

28. `Halt` — останавливает работу программы
    - Снимает значение со стека вычислений, используя его как целочисленный код возврата
    - Является последней инструкцией любой программы (либо `Return`, либо `Jump`)

## Встроенные функции

Встроенные функции вызываются инструкцией `CallBuiltin`. Аргументы передаются через стек вычислений. Возвращаемое значение помещается в стек вычислений.

| Код | Имя | Сигнатура | Описание |
|-----|-----|-----------|----------|
| 1 | `Print` | `print(value)` | Снимает `EVAL[^1]` и выводит его в stdout. `bool` выводится как `1` (true) или `0` (false) |
| 2 | `ReadI` | `readi() → int` | Читает целое число из stdin, помещает результат в стек вычислений |
| 3 | `ReadF` | `readf() → float` | Читает вещественное число из stdin, помещает результат в стек вычислений |
| 4 | `ReadS` | `reads() → string` | Читает строку из stdin, помещает результат в стек вычислений |
| 5 | `Length` | `length(string) → int` | Снимает `EVAL[^1]`, помещает в стек длину строки в Unicode code points как `int` |
| 6 | `Substr` | `substr(s: string, start: int, length: int) → string` | Снимает `EVAL[^3]` (длина), `EVAL[^2]` (начало), `EVAL[^1]` (строка). Возвращает подстроку в code points |